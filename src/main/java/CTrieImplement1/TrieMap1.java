...
public class TrieMap1<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K,V>, Serializable {
    private static final AtomicReferenceFieldUpdater<TrieMap1, Object> ROOT_UPDATER = AtomicReferenceFieldUpdater.newUpdater(TrieMap1.class, Object.class, "root");
    ...
    static {
        final Field f;
        try {
            f = TrieMap1.class.getDeclaredField("readOnly");
    ...
    public static <K,V> TrieMap1<K,V> empty () {
        return new TrieMap1<K, V>();
    }
    ...
    static class INode<K, V> extends INodeBase<K, V> {
        ...
        final MainNode<K, V> gcasRead (final TrieMap1<K, V> ct) {
            return GCAS_READ (ct);
        }

        final MainNode<K, V> GCAS_READ (TrieMap1<K, V> ct) {
        ...
        private MainNode<K, V> GCAS_Complete (MainNode<K, V> m, final TrieMap1<K, V> ct) {
            while (true) {
                if (m == null)
                    return null;
                else {
        ...
        final boolean GCAS (final MainNode<K, V> old, final MainNode<K, V> n, final TrieMap1<K, V> ct) {
            n.WRITE_PREV (old);
            if (CAS (old, n)) {
                GCAS_Complete (n, ct);
        ...
        private boolean equal (final K k1, final K k2, final TrieMap1<K, V> ct) {
        ...
        final INode<K, V> copyToGen (final Gen ngen, final TrieMap1<K, V> ct) {
         ...
         */
        final boolean rec_insert (final K k, final V v, final int hc, final int lev, final INode<K, V> parent, final Gen startgen, final TrieMap1<K, V> ct) {
            while (true) {
         ...
         */
        final Option<V> rec_insertif (final K k, final V v, final int hc, final Object cond, final int lev, final INode<K, V> parent, final Gen startgen, final TrieMap1<K, V> ct) {
            while (true) {
                MainNode<K, V> m = GCAS_READ (ct); // use -Yinline!
...
//                throw new RuntimeException ("Should not happen");
        ...
        final boolean insertln (final LNode<K, V> ln, final K k, final V v, final TrieMap1<K, V> ct) {
         ...
         */
        final Object rec_lookup (final K k, final int hc, int lev, INode<K, V> parent, final Gen startgen, final TrieMap1<K, V> ct) {
            while (true) {
                MainNode<K, V> m = GCAS_READ (ct); // use -Yinline!
        ...
        private Object cleanReadOnly (final TNode<K, V> tn, final int lev, final INode<K, V> parent, final TrieMap1<K, V> ct, K k, int hc) {
            if (ct.nonReadOnly ()) {
                clean (parent, ct, lev - 5);
         ...
         *         value otherwise
         */
        final Option<V> rec_remove (K k, V v, int hc, int lev, final INode<K, V> parent, final Gen startgen, final TrieMap1<K, V> ct) {
            MainNode<K, V> m = GCAS_READ (ct); // use -Yinline!
        ...
        void cleanParent (final Object nonlive, final INode<K, V> parent, final TrieMap1<K, V> ct, final int hc, final int lev, final Gen startgen) {
            while (true) {
        ...
        private void clean (final INode<K, V> nd, final TrieMap1<K, V> ct, int lev) {
        ...
        final boolean isNullInode (final TrieMap1<K, V> ct) {
            return GCAS_READ (ct) == null;
        }

        final int cachedSize (final TrieMap1<K, V> ct) {
    ...
    private final static class LNode<K, V> extends MainNode<K, V> {
        ...
        MainNode<K, V> removed (K k, final TrieMap1<K, V> ct) {
    ...
    private static final class CNode<K, V> extends CNodeBase<K, V> {
        ...
        final public int cachedSize (Object ct) {
            ...
            if (currsz != -1)
                ...
                int sz = computeSize ((TrieMap1<K, V>) ct);
                while (READ_SIZE () == -1)
                    CAS_SIZE (-1, sz);
                return READ_SIZE ();
            }
        ...
        // to be independent
        private int computeSize (final TrieMap1<K, V> ct) {
            int i = 0;
            int sz = 0;
         ...
         */
        final CNode<K, V> renewed (final Gen ngen, final TrieMap1<K, V> ct) {
            int i = 0;
            BasicNode[] arr = array;
            int len = arr.length;
        ...
        final MainNode<K, V> toCompressed (final TrieMap1<K, V> ct, int lev, Gen gen) {
            int bmp = bitmap;
            int i = 0;
            BasicNode[] arr = array;
    ...
    TrieMap1(final Hashing<K> hashf, final Equiv<K> ef, final boolean readOnly) {
        this.hashingobj = hashf;
        this.equalityobj = ef;
        this.readOnly = readOnly;
    }

    TrieMap1(final Object r, final Hashing<K> hashf, final Equiv<K> ef, boolean readOnly) {
        this(hashf, ef, readOnly);
        this.root = r;
    }

    public TrieMap1(final Hashing<K> hashf, final Equiv<K> ef) {
    ...
    final public TrieMap1<K, V> snapshot () {
        while (true) {
            INode<K, V> r = RDCSS_READ_ROOT ();
            final MainNode<K, V> expmain = r.gcasRead (this);
            if (RDCSS_ROOT (r, expmain, r.copyToGen (new Gen (), this)))
                return new TrieMap1<K, V>(r.copyToGen (new Gen (), this), hashing (), equality (), readOnly);
            else {
                // return snapshot ();
                // tailrec
                continue;
     ...
     */
    final public TrieMap1<K, V> readOnlySnapshot () {
        ...
        while (true) {
            ...
            if (RDCSS_ROOT (r, expmain, r.copyToGen (new Gen (), this)))
                return new TrieMap1<K, V>(r, hashing (), equality (), true);
            else {
...
//    }
    ...
    final public TrieMap1<K, V> add (K k, V v) {
        update (k, v);
        return this;
    }
...
//    }
    ...
    private static class TrieMapReadOnlyIterator<K, V> extends TrieMapIterator<K, V> {
        TrieMapReadOnlyIterator (int level, final TrieMap1<K, V> ct, boolean mustInit) {
            super (level, ct, mustInit);
        }

        TrieMapReadOnlyIterator (int level, TrieMap1<K, V> ct) {
            this (level, ct, true);
        }        
        void initialize () {
    ...
    private static class TrieMapIterator<K, V> implements java.util.Iterator<Map.Entry<K, V>> {
        ...
        TrieMapIterator (int level, final TrieMap1<K, V> ct, boolean mustInit) {
            this.level = level;
            this.ct = ct;
        ...
        TrieMapIterator (int level, TrieMap1<K, V> ct) {
            this (level, ct, true);
        }
...
//            assert (ct.isReadOnly ());
        ...
        protected TrieMapIterator<K, V> newIterator (int _lev, TrieMap1<K, V> _ct, boolean _mustInit) {
    ...
    final class EntrySet extends AbstractSet<Map.Entry<K, V>> {
        ...
        public final boolean remove (final Object o) {
            ...
            final Map.Entry<K, V> e = (Map.Entry<K, V>) o;
            final K k = e.getKey ();
            return null != TrieMap1.this.remove (k);
        }
