 ...
 */
public final class MutableTrieMap<K, V> extends TrieMap2<K, V> {
    ...
    public V putIfAbsent(final K key, final V value) {
        final K k = requireNonNull(key);
        return toNullable(insertifhc(k, computeHash(k), requireNonNull(value), PresencePredicate.ABSENT));
    }
    ...
    public V replace(final K key, final V value) {
        final K k = requireNonNull(key);
        return toNullable(insertifhc(k, computeHash(k), requireNonNull(value), PresencePredicate.PRESENT));
    }
