...
/**
 * Abstract base class for implementing {@link TrieMap2} entry sets.
 *
 * @author Robert Varga
 *
...
abstract class AbstractEntrySet<K, V> extends AbstractSet<Entry<K, V>> {
    private final TrieMap2<K, V> map;

    AbstractEntrySet(final TrieMap2<K, V> map) {
        this.map = requireNonNull(map);
    }

    final TrieMap2<K, V> map() {
        return map;
    }
