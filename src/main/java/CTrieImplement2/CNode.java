...
import static CTrieImplement2.Constants.HASH_BITS;
...
final class CNode<K, V> extends MainNode<K, V> {
     ...
     */
    CNode<K, V> renewed(final Gen ngen, final TrieMap2<K, V> ct) {
        int idx = 0;
    ...
    MainNode<K, V> toCompressed(final TrieMap2<?, ?> ct, final int lev, final Gen newGen) {
        int bmp = bitmap;
        int idx = 0;
        BasicNode[] arr = array;
