 ...
 */
package CTrieImplement2;

import static CTrieImplement2.Constants.LEVEL_BITS;
...
final class INode<K, V> extends BasicNode {
    ...
    MainNode<K, V> gcasRead(final TrieMap2<?, ?> ct) {
    ...
    private MainNode<K, V> gcasComplete(final MainNode<K, V> oldmain, final TrieMap2<?, ?> ct) {
    ...
    private boolean gcas(final MainNode<K, V> oldMain, final MainNode<K, V> newMain, final TrieMap2<?, ?> ct) {
    ...
    INode<K, V> copyToGen(final Gen ngen, final TrieMap2<?, ?> ct) {
    ...
    private boolean recInsert(final K key, final V val, final int hc, final int lev, final INode<K, V> parent,
            final Gen startgen, final TrieMap2<K, V> ct) {
        while (true) {
    ...
    private Optional<V> insertDual(final TrieMap2<K, V> ct, final CNode<K, V> cn, final int pos, final SNode<K, V> sn,
    ...
    Optional<V> recInsertIf(final K key, final V val, final int hc, final Object cond, final int lev,
            final INode<K, V> parent, final TrieMap2<K, V> ct) {
    ...
    private Optional<V> recInsertIf(final K key, final V val, final int hc, final Object cond, final int lev,
            final INode<K, V> parent, final Gen startgen, final TrieMap2<K, V> ct) {
        while (true) {
            final MainNode<K, V> m = gcasRead(ct);
            ...
            if (m instanceof CNode) {
                ...
                if ((bmp & flag) != 0) {
                    ...
                    if (cnAtPos instanceof INode) {
                        ...
                        } else if (cond == PresencePredicate.ABSENT) {
                            if (sn.hc == hc && ct.equal(sn.key, key)) {
                                return Optional.of(sn.value);
                            }

                            return insertDual(ct, cn, pos, sn, key, val, hc, lev);
                        } else if (cond == PresencePredicate.PRESENT) {
                            if (sn.hc == hc && ct.equal(sn.key, key)) {
                                if (gcas(cn, cn.updatedAt(pos, new SNode<>(key, val, hc), gen), ct)) {
                                    return Optional.of(sn.value);
                                }
                ...
                } else if (cond == null || cond == PresencePredicate.ABSENT) {
                    final CNode<K, V> rn = cn.gen == gen ? cn : cn.renewed(gen, ct);
    ...
    private boolean insertln(final LNode<K, V> ln, final K key, final V val, final TrieMap2<K, V> ct) {
        return gcas(ln, ln.insertChild(key, val), ct);
    }

    private boolean replaceln(final LNode<K, V> ln, final LNodeEntry<K, V> entry, final V val, final TrieMap2<K, V> ct) {
     ...
     */
    Object recLookup(final K key, final int hc, final int lev, final INode<K, V> parent, final TrieMap2<K, V> ct) {
    ...
    private Object cleanReadOnly(final TNode<K, V> tn, final int lev, final INode<K, V> parent,
                                 final TrieMap2<K, V> ct, final K key, final int hc) {
        if (ct.isReadOnly()) {
    ...
    private void cleanParent(final Object nonlive, final INode<K, V> parent, final TrieMap2<K, V> ct, final int hc,
    ...
    private void clean(final INode<K, V> nd, final TrieMap2<K, V> ct, final int lev) {
